setMethod("gating", signature = c("gatingTemplate", "GatingSetInternal"), definition = function(x, y, ...) {
  ...
  for (i in 1:nrow(node_ids)) {
    ...
    for (gate in unique(gates)) {
      ...
      #get population info
      pops<-lapply(cur_gt_children_ids, function(gt_children_id) {
        getNodes(gt, gt_children_id)
      })
      # pass the pops and gate to gating routine
      gs_node_ids <- gating(x = gate, y, parent = as.integer(gs_parent_id),
                            gtPops = pops, ...)
...
setMethod("gating", signature = c("gtMethod", "GatingSet"),
          definition = function(x, y, gtPops, parent, num_nodes = 1,
