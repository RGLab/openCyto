...
# TODO: Calculate priors only when applying the corresponding gate.
# --OR--
# Move the call to elicit priors to the gating functions.
setMethod("gating", signature = c("HVTN080", "GatingSet"), definition = function(x, wf, ...) {
  ...
  channel_vMarkers <- markers2channels(getData(gating_set)[[1]], vMarkers(x))

  # Elicitation of priors for the Singlet gate
  prior_singlet <- list()
  prior_singlet$xChannel <- prior_flowClust1d(flow_set = getData(gating_set, parent),
                                              channel = channel_vMarkers, K = 3,
                                              adjust = 1.5)
  prior_singlet$yChannel <- prior_flowClust1d(flow_set = getData(gating_set, parent),
  ...
  prior_lymph <- prior_flowClust1d(flow_set = getData(gating_set, parent),
  ...
  channel_CD3 <- markers2channels(getData(gating_set)[[1]], tMarkers(x))
  prior_CD3 <- prior_flowClust1d(flow_set = getData(gating_set, parent),
  ...
  channel_tsubMarkers <- markers2channels(getData(gating_set)[[1]], tsubMarkers(x))

  prior_CD4 <- prior_flowClust1d(flow_set = getData(gating_set, parent),
                                 channel = channel_tsubMarkers[1], K = 3,
                                 adjust = 1.5)
  prior_CD8 <- prior_flowClust1d(flow_set = getData(gating_set, parent),
                                 channel = channel_tsubMarkers[2], K = 3,
                                 adjust = 1.5)
  gating.tsub(x, wf, pViewName = parent, K = 3, prior_CD4 = prior_CD4,
              prior_CD8 = prior_CD8, ...)
  gc()
  ...
  channel_CD57 <- markers2channels(getData(gating_set)[[1]], "CD57")
  prior_CD57 <- prior_flowClust1d(flow_set = getData(gating_set, parent),
  ...
  channel_Perforin <- markers2channels(getData(gating_set)[[1]], "Perforin")
  channel_GzB <- markers2channels(getData(gating_set)[[1]], "Granzyme B")
  prior_Perforin <- prior_flowClust1d(flow_set = getData(gating_set, parent),
                                      channel = channel_Perforin, K = 3)
  prior_GzB <- prior_flowClust1d(flow_set = getData(gating_set, parent),
  ...
  channel_IL2 <- markers2channels(getData(gating_set)[[1]], "IL2")
  channel_IFNg <- markers2channels(getData(gating_set)[[1]], "IFNg")
  prior_IL2 <- prior_flowClust1d(flow_set = getData(gating_set, parent),
                                      channel = channel_IL2, K = 3)
  prior_IFNg <- prior_flowClust1d(flow_set = getData(gating_set, parent),
  ...
  channel_TNFa <- markers2channels(getData(gating_set)[[1]], "TNFa")
  prior_TNFa <- prior_flowClust1d(flow_set = getData(gating_set, parent),
                                  channel = channel_TNFa, K = 3)
  gating.TNFa(x, wf, pViewName = parent, K = 3, prior = prior_TNFa,
              cutpoint_method = "quantile", quantile = 0.999, ...)

  # TODO: Construct polyfunctional gates following manual gating strategy
  #  gating.polyfunction(x, wf, pViewName = parent, ...)
  ...
  # TODO: Construct 'not CD4' gate

  parent <- "not CD4+"
  
  # TODO: Elicit 1D prior for IFNg
  # TODO: Construct IFNg+ gate conditional on 'not CD4+'
...
gating.CD57 <- function(x, wf, pViewName, plot = FALSE, batch = FALSE, xbin = 128,
  ...
  if (step >= 3) {
    ...
    # CD57+ gates
    CD57_pos.filterList <- Gating1D(curData, y = xChannel, filterId = "CD57+",
                                    positive = TRUE, nslaves = nslaves,
                                    method = "flowClust", usePrior = usePrior, ...)

    # CD57- gates (the negation of the CD57+ gates)
    CD57_neg.filterList <- lapply(CD57_pos.filterList, function(gate) {
      gate@max <- gate@min
      gate@min <- -Inf
      gate@filterId <- "CD57-"
      gate
    })
    CD57_neg.filterList <- filterList(CD57_neg.filterList)
    
    nodeID <- add(wf, CD57_pos.filterList, parent = pViewName)
    recompute(wf, nodeID)

    nodeID <- add(wf, CD57_neg.filterList, parent = pViewName)
    recompute(wf, nodeID)
...
gating.Perforin_GzB <- function(x, wf, pViewName, plot = FALSE, batch = FALSE,
  ...
  if (step >= 3) {
    ...
    GzB_pos.list <- lapply(Perforin_GzB.filterList, function(filter_i) {
      filter_i[["Perforin-Granzyme B+"]]@filterId <- "GzB+"
      filter_i[["Perforin-Granzyme B+"]]
    })
    GzB_pos.list <- filterList(GzB_pos.list)

    GzB_neg.list <- lapply(Perforin_GzB.filterList, function(filter_i) {
      filter_i[["Perforin-Granzyme B-"]]@filterId <- "GzB-"
      filter_i[["Perforin-Granzyme B-"]]
    })
    GzB_neg.list <- filterList(GzB_neg.list)


    nodeID <- add(wf, Perforin.list, parent = pViewName)
    recompute(wf, nodeID)

    nodeID <- add(wf, GzB_pos.list, parent = pViewName)
    recompute(wf, nodeID)

    nodeID <- add(wf, GzB_neg.list, parent = pViewName)
    recompute(wf, nodeID)
...
gating.TNFa <- function(x, wf, pViewName, plot = FALSE, batch = FALSE, xbin = 128,
  ...
  if (step >= 3) {
    ...
    TNFa_pos.filterList <- Gating1D(curData, y = xChannel, filterId = filterId,
                                    positive = TRUE, nslaves = nslaves, method = "flowClust",
                                    usePrior = usePrior, ...)
